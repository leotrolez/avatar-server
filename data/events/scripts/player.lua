...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
	...
	-- TODO: wtf
	sendResetQuestRequisitions(item, toPosition)
...
function Player:onMoveCreature(creature, fromPosition, toPosition)
	...
	if getPlayerStorageValue(self, "canAttackable") == 1 and getPlayerStorageValue(creature, "canAttackable") ~= 1 then 
		return false
	end
...
function Player:onTurn(direction)
	...
	if getCreatureNoMove(cid) then
		if getPlayerStorageValue(cid, "playerCanMoveDirection") < os.time() then
			return false
		end
	end
	return true
...
function Player:onTradeRequest(target, item)
	...
	if not isMovable(item.uid) or item.itemid == 9775 then
		doPlayerSendCancel(cid, getLangString(cid, "You cannot trade imovable things.", "Você não pode negociar itens imoveis."))
		return false
	end

	if item.itemid == 12325 then --item do reset (nao funfa, feito pela source)
		doPlayerSendCancel(cid, getLangString(cid, "You cannot trade this item.", "Você não pode iniciar trade com esse item."))
		return false
	end
...
local errorMsg = {
	["fire"] = {"Before you need learn this fold.", "Você ainda não aprendeu a usar essa dobra."},
	["air"] = {"Before you need learn this fold.", "Você ainda não aprendeu a usar essa dobra."},
	["earth"] = {"Before you need learn this fold.", "Você ainda não aprendeu a usar essa dobra."},
	["water"] = {"Before you need learn this fold.", "Você ainda não aprendeu a usar essa dobra."}
}

function Player:onCastSpell(spell)
	local cid = self:getId()
	...
	if getPlayerStorageValue(cid, "hasActiveInQuest") == 1 then
		for x = 1, 20 do
			local currentSpell = getPlayerStorageValue(cid, "canUseSpell"..x)
			
			if not (currentSpell == -1) then
				if name == currentSpell then
					doPlayerSendCancel(cid, getLangString(cid, "You cannot use this fold here.", "Você não pode usar essa dobra aqui."))
					return false
				end
			else
				break
