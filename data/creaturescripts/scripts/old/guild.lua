function onChannelJoin(cid, channelId, users)
  if(channelId ~= CHANNEL_GUILD) then
   ...
   for _, pid in pairs(users) do
    doPlayerSendChannelMessage(pid, "Information", "Player " .. getCreatureName(cid) .. " has entered the channel.", TALKTYPE_CHANNEL_HIGHLIGHT, CHANNEL_GUILD)
  end
...
function onChannelLeave(cid, channelId, users)
  ...
  for _, pid in users do
    doPlayerSendChannelMessage(pid, "Information", "Player " .. getCreatureName(cid) .. " has left the channel.", TALKTYPE_CHANNEL_HIGHLIGHT, CHANNEL_GUILD)
  end
