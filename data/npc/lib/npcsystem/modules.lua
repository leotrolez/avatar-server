...
if Modules == nil then
	...
	function ShopModule:addBuyableItem(names, cost, itemid, itemSubType, realName)
		if type(names) ~= "table" then
			names = { names }
		end
		if realName then
			realName = string.len(realName) > 0 and realName or nil
		end
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
	...
	--	subType - The subType of each rune or fluidcontainer item. Can be left out if it is not a rune/fluidcontainer. Default value is 1.
	--	realName - The real, full name for the item. Will be used as ITEMNAME in MESSAGE_ONBUY and MESSAGE_ONSELL if defined. Default value is nil (ItemType(itemId):getName() will be used)
	function ShopModule:addBuyableItemContainer(names, container, cost, itemid, subType, realName)
		if names then
			if type(names) ~= "table" then
				names = { names }
			end
	...
	--	cost = The price of one single item
	--	realName - The real, full name for the item. Will be used as ITEMNAME in MESSAGE_ONBUY and MESSAGE_ONSELL if defined. Default value is nil (ItemType(itemId):getName() will be used)
	function ShopModule:addSellableItem(names, cost, itemid, realName, itemSubType)
		if type(names) ~= "table" then
			names = { names }
		end
		if realName then
