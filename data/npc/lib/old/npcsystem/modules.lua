...
if(Modules == nil) then
  ...
  function TravelModule:parseParameters()
    ...
    if(ret ~= nil) then
      ...
      self.npcHandler.keywordHandler:addKeyword({'rotas'}, TravelModule.listDestinations, {module = self})
      self.npcHandler.keywordHandler:addKeyword({'routes'}, TravelModule.listDestinations, {module = self})
  ...
  function ShopModule:parseSellable(data)
    for item in string.gmatch(data, '[^;]+') do
      ...
      if(SHOPMODULE_MODE == SHOPMODULE_MODE_TRADE) then
        if(itemid ~= nil and cost ~= nil) then
          self:addSellableItem(nil, cost, itemid, name)
        else
          print('[Warning] NpcSystem:', 'Parameter(s) missing for item:', itemid, cost)
        end
      else
        if(name ~= nil and itemid ~= nil and cost ~= nil) then
          local names = {}
          table.insert(names, name)
          self:addSellableItem(names, cost, itemid, name)
        else
        ---  print('[Warning] NpcSystem:', 'Parameter(s) missing for item:', name, itemid, cost)
        end
      end
  ...
  function ShopModule:addBuyableItem(names, cost, itemid, subType, realName)
    ...
    if(SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK) then
      ...
      self.npcHandler.shopItems[itemid].realName = names[1] or getItemNameById(itemid)
    ...
    if(names ~= nil and SHOPMODULE_MODE ~= SHOPMODULE_MODE_TRADE) then
      for i, name in pairs(names) do
        local parameters = {
            ...
            module = self,
            realName = names[1] or getItemNameById(itemid),
  ...
  --  cost = The price of one single item
  --  realName - The real, full name for the item. Will be used as ITEMNAME in MESSAGE_ONBUY and MESSAGE_ONSELL if defined. Default value is nil (getItemNameById will be used)
  function ShopModule:addSellableItem(names, cost, itemid, realName)
	if type(names) == "string" then 
		if realName == '' then 
			realName = names
		end 
  ...
  function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
	...
	local theBuyPrice = self.npcHandler.shopItems[itemid].buyPrice
	if (getCreatureName(getNpcId()) == "Edward Colin" or getCreatureName(getNpcId()) == "Hugo" or getCreatureName(getNpcId()) == "Borkas") and getPlayerResets(cid) >= 50 then
		theBuyPrice = theBuyPrice*0.8
	end

    local backpack = 1988
    local totalCost = amount * theBuyPrice
    if(inBackpacks) then
    ...
    if(a < amount) then
      ...
      if(a > 0) then
        doPlayerRemoveMoney(cid, ((a * theBuyPrice) + (b * 20)))
        return true
      end
  ...
  function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreCap, inBackpacks)
	...
	local theSellPrice = self.npcHandler.shopItems[itemid].sellPrice
	if (getCreatureName(getNpcId()) == "Mithlock" or getCreatureName(getNpcId()) == "Kurt") and getPlayerResets(cid) >= 20 then
		theSellPrice = theSellPrice*1.2
	end
    local parseInfo = {
      [TAG_PLAYERNAME] = getPlayerName(cid),
      [TAG_ITEMCOUNT] = amount,
      [TAG_TOTALCOST] = amount * theSellPrice,
      [TAG_ITEMNAME] = self.npcHandler.shopItems[itemid].realName
    }
    ...
    if(doPlayerRemoveItem(cid, itemid, amount, subType)) then
      ...
      msg = self.npcHandler:parseMessage(msg, parseInfo)
      doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, msg)
      doPlayerAddMoney(cid, amount * theSellPrice)
      if(NPCHANDLER_CONVBEHAVIOR ~= CONVERSATION_DEFAULT) then
        self.npcHandler.talkStart[cid] = os.time()
      else
        self.npcHandler.talkStart = os.time()
  ...
  function ShopModule.requestTrade(cid, message, keywords, parameters, node)
	...
	local desconto = false
	local descontoBuy = false
	if (getCreatureName(getNpcId()) == "Mithlock" or getCreatureName(getNpcId()) == "Kurt") and getPlayerResets(cid) >= 20 then
		desconto = true
	end
	if (getCreatureName(getNpcId()) == "Edward Colin" or getCreatureName(getNpcId()) == "Hugo" or getCreatureName(getNpcId()) == "Borkas") and getPlayerResets(cid) >= 50 then
    ...
    for itemid, attr in pairs(module.npcHandler.shopItems) do
	  ...
	  if desconto and type(item.sell) == "number" then
		item.sell = item.sell * 1.2
	  end
	  if descontoBuy and type(item.buy) == "number" then
		item.buy = item.buy * 0.8
	  end
  ...
  function ShopModule.onConfirm(cid, message, keywords, parameters, node)
	...
	local theCost = parentParameters.cost
    if(parentParameters.eventType == SHOPMODULE_SELL_ITEM) then
      if (getCreatureName(getNpcId()) == "Mithlock" or getCreatureName(getNpcId()) == "Kurt") and getPlayerResets(cid) >= 20 then
		theCost = theCost * 1.2
		parseInfo[TAG_TOTALCOST] = theCost * module.amount
	  end
      local ret = doPlayerSellItem(cid, parentParameters.itemid, module.amount, theCost * module.amount)
      if(ret) then
    ...
    elseif(parentParameters.eventType == SHOPMODULE_BUY_ITEM) then
      ...
      local ret = doPlayerBuyItem(cid, parentParameters.itemid, module.amount, theCost * module.amount, parentParameters.subType)
      if(ret) then
  ...
  function ShopModule.tradeItem(cid, message, keywords, parameters, node)
    ...
    if(parameters.eventType == SHOPMODULE_SELL_ITEM) then
		if (getCreatureName(getNpcId()) == "Kurt" or getCreatureName(getNpcId()) == "Mithlock") and getPlayerResets(cid) >= 20 then
			parseInfo[TAG_TOTALCOST] = parseInfo[TAG_TOTALCOST]*1.2
		end
    ...
    elseif(parameters.eventType == SHOPMODULE_BUY_ITEM) then
		if (getCreatureName(getNpcId()) == "Edward Colin" or getCreatureName(getNpcId()) == "Hugo" or getCreatureName(getNpcId()) == "Borkas") and getPlayerResets(cid) >= 50 then
			parseInfo[TAG_TOTALCOST] = parseInfo[TAG_TOTALCOST]*0.8
		end
