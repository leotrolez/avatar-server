...
doBroadcastMessage = Game.broadcastMessage
...
function setCombatParam(combat, type, value)
	if type == COMBAT_PARAM_EFFECT or type == COMBAT_PARAM_DISTANCEEFFECT then
		value = value + 1
	end

	combat:setParameter(type, value)
end
...
function isCreature(cid) return Creature(cid) and true or false end
function isPlayer(cid) return Player(cid) and true or false end
function isMonster(cid) return Monster(cid) and true or false end
function isSummon(cid) local c = Creature(cid) return c and c:getMaster() end
function isNpc(cid) return Npc(cid) and true or false end
function isItem(uid) return Item(uid) and true or false end
function isContainer(uid) return Container(uid) and true or false end
...
doPlayerAddOutfitId = function()
	-- TODO: implement
end

function doPlayerCancelFollow(cid) local c = Creature(cid) return c and c:setFollowCreature(nil) or false end
...
function doCreatureSetNoMove(cid, block, ...)
	local c = Creature(cid)
	if c then
		return c:setMovementBlocked(block, ...)
	end
	return false
end
...
function hasCondition(cid, type) local c = Creature(cid) return c and c:hasCondition(type) or false end
...
function doPlayerAddMagicLevel(cid, amount) local p = Player(cid) return p and p:addMagicLevel(amount) or false end
doPlayerAddMagLevel = doPlayerAddMagicLevel
...
function getPlayerStorageValue(cid, key) 
	local c = Creature(cid) 
	if c and (c:isPlayer() or c:isMonster()) then
		return c:getStorageValue(key)
	end
...
function setPlayerStorageValue(cid, key, value) 
	local c = Creature(cid) 
	if c and (c:isPlayer() or c:isMonster()) then
		return c:setStorageValue(key, value)
	end
...
function doSendAnimatedText(position, text, color, cid) Game.sendAnimatedText(text, position, color, cid and Player(cid) or nil) end
...
function doSendPlayerExtendedOpcode(cid, opcode, buffer) local p = Player(cid) return p and p:sendExtendedOpcode(opcode, buffer) or false end
...
function doSendMagicEffect(pos, magicEffect, ...) return Position(pos):sendMagicEffect(magicEffect + 1, ...) end
function doSendDistanceShoot(fromPos, toPos, distanceEffect, ...) return Position(fromPos):sendDistanceEffect(toPos, distanceEffect + 1, ...) end
...
function getHouseFromPos(position) 
...
function getPositionByDirection(positionx, direction, size)
  local position = positionx
  local n = size or 1
...
function getPlayerResets(cid)
	return math.max(0, getPlayerStorageValue(cid, PlayerStorageKeys.resets))
end

function setPlayerResets(cid, amount)
	return setPlayerStorageValue(cid, PlayerStorageKeys.resets, amount)
end
...
function getPlayerModes(cid)
	...
	local mode = player:getFightMode()
	local tb = {
		["chase"] = mode == 1 and true or false,
		["fight"] = mode == 2 and true or false,
		["secure"] = mode == 3 and true or false,
	}
	return tb
end
...
function doCreateItem(itemid, count, pos)
	local typeCount = type(count)
	if typeCount == "table" or typeCount == "userdata" then
		pos = count
		count = 1
	end
...
function getItemAttribute(uid, key)
	...
	return item:getCustomAttribute(key) or false
end
...
function doItemSetAttribute(uid, key, value)
	...
	return item:setCustomAttribute(key, value)
end