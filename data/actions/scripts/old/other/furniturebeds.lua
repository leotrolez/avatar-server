...
local function internalBedTransform(item, itemEx, toPosition, ids, cid)
	doTransformItem(itemEx.uid, ids[1])
		doCleanTile(toPosition, true)

	doCreateItem(ids[2], 1, toPosition)
...
--[[	local newBed = BEDS[item.itemid]
	...
	for kit, bed in pairs(BEDS) do
		if(bed[1][1] == itemEx.itemid or itemEx.itemid == 1758) then
			...
			internalBedTransform(item, itemEx, toPosition, newBed[1], cid)
			break
		elseif(bed[2][1] == itemEx.itemid or itemEx.itemid == 1756) then
			toPosition.x = toPosition.x + 1
			internalBedTransform(item, itemEx, toPosition, newBed[2], cid)
			break
		end
	end
	]]
