...
function config.start(cid)
    ...
    for x = 1, #config.posesToEnter do
        ...
        if possiblePlayer > 0 then
            ...
            doPlayerSendCancel(cid, getLangString(cid, "Your team needs have four players to start Anihilator.", "O seu time precisa de quatro players para iniciar a Anihilator."))
            return false
        end
    end
...
function onUse(creature, item, fromPosition, itemEx, toPosition)
  local cid = creature.uid
    ...
    if dVar.timeToDoAgain < currentTime then
        ...
        doPlayerSendCancel(cid, getLangString(cid, "You need wait "..getSecsString(dVar.timeToDoAgain-currentTime).." to start anihilator.", "Você precisa esperar "..getSecsString(dVar.timeToDoAgain-currentTime).." para iniciar a Anihilator."))
    end
