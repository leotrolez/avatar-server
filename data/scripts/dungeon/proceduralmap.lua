local function IsInMapRange(map_select, x, y)
...
local Styles = {
    ["Rock Mountain"] = {
        Wall = {919},   --wall
        Ground = {4405,4406,4407,4408,4409,4410,4411,4417,4413,4414,4415,4416,4417,4418,4419,4420,4421},   --Ground
        GroundBorder = {4405,4406,4407,4408,4409,4410,4411,4417,4413,4414,4415,4416,4417,4418,4419,4420,4421},
        N = 4471,     --n
        W = 4472,     --w
        S = 4469,     --s
        E = 4468,     --e
    ...
    ["Earth"] = {
        ...
        Ground = {351,351,351,351,351,352,353,354,355},   --Ground
        GroundBorder = {351,351,351,351,351,352,353,354,355},
        N = 5632,     --n
        W = 5631,     --w
        S = 5638,     --s
        E = 5637,     --e
    ...
    ["Earth Mountain"] = {
        Wall = {8133},   --wall
        Ground = {106,106,106,106,106,108,109},   --Ground
        GroundBorder = {106,106,106,106,106,108,109},
        N = 7989,     --n
        W = 7988,     --w
        S = 8169,     --s
        E = 8168,     --e
    ...
    ["Darkest Mud (lava)"] = {
        ...
        Ground = {11505,11506,11507,11508,11509},
        GroundBorder = {598,598,598,598,598,599,600,601},
        N = 11524,
        W = 11523,
        S = 11522,
        E = 11525,
...
local StylesList = {"Darkest Mud (lava)", "Earth Mountain", "Earth (stone border)", "Earth (stone border)", "Earth", "Rock Mountain"}

local MAPS = {
    [1] = {
        startX = 580, startY = 2049, startZ = 6,
        width = 155, height = 155,
        randomFillPercent = 55,
        roomTheresholdSize = 10,
        wallTheresholdSize = 90,
        map = {},
        removeFirstLayer = true,
        style = Styles["Earth"],
        spawnId = 1,
        monsters = {
            {name = "Rotworm", amount = 25},
            {name = "Clawed Rotworm", amount = 25},
        },
        boss = {
            name = "Rotworm Queen"
        }
...
MapGeneration = MapGeneration or {}

MapMonstersSpawns = MapMonstersSpawns or {}

function MapGeneration.ProcessMap(map_select)
    local wallRegions = MapGeneration.GetRegions(map_select, 1)
    ...
    local roomRegions = MapGeneration.GetRegions(map_select, 0)
    ...
    if survivingRooms[1] then
        survivingRooms[1].isAccessibleFromMainRoom = true
        survivingRooms[1].isMainRoom = true
    end

    --print("allRooms: "..#survivingRooms)
    map_select.map = MapGeneration.ConnectClosestRooms(map_select, survivingRooms)
    return map_select.map
end

function MapGeneration.ConnectClosestRooms(map_select, allRooms, forceAccessibilityFromMainRoom)
    ...
    for i, roomA in ipairs(roomListA) do
        ...
        if possibleConnectionFound and not forceAccessibilityFromMainRoom then
            map_select.map = MapGeneration.CreatePassage(map_select, bestRoomA, bestRoomB, bestTileA, bestTileB)
        end
    ...
    if possibleConnectionFound and forceAccessibilityFromMainRoom then
        map_select.map = MapGeneration.CreatePassage(map_select, bestRoomA, bestRoomB, bestTileA, bestTileB)
        map_select.map = MapGeneration.ConnectClosestRooms(map_select, allRooms, true)
    end
...
function MapGeneration.CreatePassage(map_select, roomA, roomB, tileA, tileB)
    ConnectRooms(roomA,roomB)
    --map_select.map[tileA.tileX][tileA.tileY] = 2
    --map_select.map[tileB.tileX][tileB.tileY] = 2

    local line = MapGeneration.GetLine(tileA,tileB)

    for index, c in ipairs(line) do
        --map[c.tileX][c.tileY] = 2
        map_select.map = MapGeneration.DrawCircle(map_select, c, 2)
    end

    return map_select.map
end

function MapGeneration.DrawCircle(map_select, c, r)
    for x = -r, r, 1 do
        for y = -r, r, 1 do
            if (x*x + y*y <= r*r) then
...
function MapGeneration.GetLine(from,to)
    local line = {}
    local x = from.tileX
    local y = from.tileY
...
function MapGeneration.GetRegions(map_select, tileType)
    local regions = {}
    local mapFlags = {}
    ...
    for x = 0, map_select.width, 1 do
        for y = 0, map_select.height, 1 do
            if mapFlags[x][y] == 0 and map_select.map[x][y] == tileType then
                newRegion = MapGeneration.GetRegionTiles(map_select, x, y)
...
function MapGeneration.GetRegionTiles(map_select, startX, startY)
    local tiles = {}
    local mapFlags = {}
    for x = 0, map_select.width, 1 do
        mapFlags[x] = {}
...
function MapGeneration.generate(map_select)
    map_select.map = MapGeneration.RandomFillMap(map_select)

    for i = 1, 5, 1 do
        map_select.map = MapGeneration.SmootMap(map_select)
    end

    -- Cria o local para o player entrar
    if map_select.playerSpawn then
    ...
    map_select.map = MapGeneration.ProcessMap(map_select)

    if map_select.removeFirstLayer then
        --remover a primeira camada de walls
        map_select.map = MapGeneration.RemoveFirstLayerWall(map_select)
    end

    return map_select.map
end

function MapGeneration.RemoveFirstLayerWall(map_select)
    local newmap = {}
    ...
    --MapGeneration.print(map_select.map, map_select.width, map_select.height)
    return newmap
end

function MapGeneration.RandomFillMap(map_select)
    map_select.map = {}

    local centerX = map_select.width / 2
    local centerY = map_select.height / 2
    ...
    for x = 0, map_select.width, 1 do
        map_select.map[x] = {}
        for y = 0, map_select.height, 1 do
            if x <= 10 or x >= map_select.width-10 or y <= 8 or y >= map_select.height-8 then
                map_select.map[x][y] = 1
            else
                map_select.map[x][y] = uniform_random(0, 100) < map_select.randomFillPercent and 1 or 0
            end
        end
    end
    return map_select.map
...
function MapGeneration.SmootMap(map_select)
    for x = 0, map_select.width, 1 do
        for y = 0, map_select.height, 1 do
            local neighbourWallTiles = MapGeneration.GetSurroundingWallCount(map_select, x, y)
...
function MapGeneration.GetSurroundingWallCount(map_select, gridX,gridY)
    local wallCount = 0
...
function MapGeneration.print(map, width, height)
    for y = 0, width, 1 do
        for x = 0, height, 1 do
            if map[x][y] == 0 then
                io.write(' ')
...
function MapGeneration.clear(mapSelect)
    for x = 0, mapSelect.width, 1 do
        for y = 0, mapSelect.height, 1 do
            -- skip player spawn
            if mapSelect.playerSpawn then
...
function MapGeneration.spawn(mapSelect)
    mapSelect.walkablePositions = {}
    for x = 0, mapSelect.width, 1 do
        for y = 0, mapSelect.height, 1 do
            -- skip player spawn
            if mapSelect.playerSpawn then
            ...
            if t then
                ...
                t = Game.createTile(mapLoc, true)
            end

            -- add cave tiles
            if mapValor == 0 then
                local tiles = MapGeneration.BorderizeWalls(mapSelect,x,y)
                if #tiles > 0 then
                    for index, tile in ipairs(tiles) do
                        Game.createItem(tile, 1, mapLoc)
                    end

                    if #mapSelect.style.GroundBorder > 1 then
                        Game.createItem(mapSelect.style.GroundBorder[uniform_random(#mapSelect.style.GroundBorder)], 1, mapLoc)
                    else
                        Game.createItem(mapSelect.style.GroundBorder[1], 1, mapLoc)
                    end
                else
                    if #mapSelect.style.Ground > 1 then
                        Game.createItem(mapSelect.style.Ground[uniform_random(#mapSelect.style.Ground)], 1, mapLoc)
                    else
                        Game.createItem(mapSelect.style.Ground[1], 1, mapLoc)
                    end
                end
            else -- add wall tiles
                if #mapSelect.style.Wall > 1 then
                    Game.createItem(mapSelect.style.Wall[uniform_random(#mapSelect.style.Wall)], 1, mapLoc)
                else
            ...
            if t and t:isWalkable() then
                table.insert(mapSelect.walkablePositions, mapLoc)
            end
...
function MapGeneration.BorderizeWalls(map_select,x,y)
    local tiles = {}
    if map_select.map[x][y] == 0 then
        local n = {}
        local cont = 0
        for neib_y = y-1,y+1, 1 do
            for neib_x = x-1,x+1, 1 do
                if neib_x > 0 and neib_x < map_select.width and neib_y > 0 and neib_y < map_select.height then
                    n[cont] = map_select.map[neib_x][neib_y] == 1
                else
                    n[cont] = true
                end
                cont = cont + 1
...
function MapGeneration.cleanupSpawns(map_select)
    local spawnId
    for x = 0, map_select.width, 1 do
        for y = 0, map_select.height, 1 do
                ...
                if creatures then
                    for index, creature in ipairs(creatures) do
                        if not creature:isPlayer() then
                            ...
                            if MapMonstersSpawns[creatureID] then
                                spawnId = MapMonstersSpawns[creatureID].spawnId
                                MapMonstersSpawns[creatureID] = nil
                            end
                            creature:remove()
                        end
                    end
    ...
    for key, m in pairs(MapMonstersSpawns) do
        if type(m) == "table" and m.spawnId == spawnId then
            MapMonstersSpawns[key] = nil
        end
    end
end

function MapGeneration.AddSpawns(map_select)
    MapMonstersSpawns.count = 0
    MapMonstersSpawns.killedCount = 0
    local tsize = #map_select.walkablePositions
    if tsize == 0 then
        return
    end
    ...
    for index, monster in ipairs(map_select.monsters) do
        for i = 1, monster.amount, 1 do
            ...
            while m == nil do
                ...
                local randpos = map_select.walkablePositions[uniform_random(tsize)]
                m = Game.createMonster(monster.name,{x=randpos.x,y=randpos.y,z=randpos.z}, true, false)
            end
            if m then
                MapMonstersSpawns[m:getId()] = {pos=m:getPosition(), spawnId=map_select.spawnId}
                MapMonstersSpawns.count = MapMonstersSpawns.count + 1
                m:registerEvent("ProceduralMonsterDeath")
                m:setStorageValue("MapGeneration", map_select.spawnId)
            end
        end
    end
end

local creatureevent = CreatureEvent("ProceduralMonsterDeath")
function creatureevent.onDeath(creature, corpse, killer, mostDamageKiller, lastHitUnjustified, mostDamageUnjustified)
    MapMonstersSpawns[creature:getId()] = nil
    MapMonstersSpawns.killedCount = MapMonstersSpawns.killedCount + 1
    if killer:isPlayer() then
        killer:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("Killed %d of %d.", MapMonstersSpawns.killedCount, MapMonstersSpawns.count))

        for key, spawn in pairs(MapMonstersSpawns) do
            local monster = Monster(key)
            if monster then
                killer:teleportTo(monster:getPosition())
                break
            end
        end
    end

    if MapMonstersSpawns.killedCount == MapMonstersSpawns.count then
        local boss = Game.createMonster(MAPS[creature:getStorageValue("MapGeneration")].boss.name, creature:getPosition(), true, true)
        if boss then
            boss:registerEvent("ProceduralBossDeath")
            m:setStorageValue("MapGeneration", creature:getStorageValue("MapGeneration"))
        end
    end
	return true
end
...
local creatureevent = CreatureEvent("ProceduralBossDeath")
function creatureevent.onDeath(creature, corpse, killer, mostDamageKiller, lastHitUnjustified, mostDamageUnjustified)
    if killer:isPlayer() then
        killer:teleportTo(killer:getTown():getTemplePosition())
        MapGeneration.clear(MAPS[creature:getStorageValue("MapGeneration")])
    end
	return true
end
...
local talk = TalkAction("/dungeon")

function talk.onSay(player, words, param)
    local MAP_SELECT = MAPS[uniform_random(#MAPS)]
    -- randomize style
    MAP_SELECT.style = Styles[StylesList[uniform_random(#StylesList)]]
    --
    local mtime = os.mtime()
    MAP_SELECT.map = MapGeneration.generate(MAP_SELECT)
    print("Generating... " .. (os.mtime() - mtime) .. "ms")
    MapGeneration.cleanupSpawns(MAP_SELECT)
    print("Cleaning spawn... " .. (os.mtime() - mtime) .. "ms")
    mtime = os.mtime()
    MapGeneration.spawn(MAP_SELECT)
    print("Spawning map... " .. (os.mtime() - mtime) .. "ms")
    mtime = os.mtime()
    MapGeneration.AddSpawns(MAP_SELECT)
    print("Spawning creatures... " .. (os.mtime() - mtime) .. "ms")
    local i = 0
    ...
    player:teleportTo(MAP_SELECT.walkablePositions[1])
	return false
end
