...
for x = 1, #MyLocal.areas do
  ...
  setCombatArea(combats[x], createCombatArea(MyLocal.areas[x]))
    ...
    function onGetPlayerMinMaxValues(cid, level, magLevel)
        local min = level+(magLevel/3)*1.0
        local max = level+(magLevel/3)*1.5
...
function onCastSpell(creature, var)
	...
	if getPlayerStorageValue(cid, "executarWhip") ~= 1 then
		return false
	else
		setPlayerStorageValue(cid, "executarWhip", -1)
	end
    local combats = {combat1, combat2, combat3, combat4, combat5, combat6, combat7, combat8}
        ...
        if MyLocal.players[cid] ~= nil or getDobrasLevel(cid) >= 1 then
			for i = 1, 4 do 
				...
				if MyLocal.removeEvent[cid] ~= nil then 
					...
					MyLocal.removeEvent[cid] = addEvent(function() removeTable(cid) MyLocal.removeEvent[cid] = nil end, 2000)
				end
			end
			return true
        else
