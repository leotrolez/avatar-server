...
local function doRemoveTable(cid)
    if isCreature(cid) then
        return doPlayerAddExaust(cid, "fire", "jump", fireExausted.jump)
        -- MyLocal.players[cid] = false
    end
end

local function sendMagicEff(cid, initPos)
    if not isCreature(cid) then
        return true
    end
    local pos = getThingPos(cid)

    if not comparePoses(pos, initPos) then
        return true
    end
...
function onCastSpell(creature, var)
    ...
    --  workAllCdAndAndPrevCd(cid, "fire", "jump", cf.duracao, 1)
    ...
    local stoneId = 8046
    local stoneId1 = 18150
    dismountPlayer(cid)
    doSendMagicEffect(position, 115)
    local debuff = 30
    ...
    if doPlayerAddUp(cid, false) then
      --doCreateTiles(459, getPlayerPosition(cid), 5000)
        sendMagicEff(cid, getThingPos(cid))
