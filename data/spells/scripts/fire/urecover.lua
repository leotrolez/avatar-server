local function purify(cid)
	...
	if hasPurify == 1 then 
		doSendAnimatedText(getCreaturePosition(cid), "Purified!", 157)
	end
end
...
function onCastSpell(creature, var)
   ...
   if doPlayerAddExaust(cid, "fire", "recover", fireExausted.recover*3) == false then
       return false
   end

    --if canUseFireSpell(cid, 1, 3, false) then
   --     workAllCdAndAndPrevCd(cid, "fire", "recover", nil, 1)
