...
setCombatParam(combat1Focus, COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE)
...
setCombatParam(combat5, COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE)

local combat6 = createCombatObject()
setCombatParam(combat6, COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE)

local combat7 = createCombatObject()
setCombatParam(combat7, COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE)

local combat8 = createCombatObject()
setCombatParam(combat8, COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE)
...
local effects = {
    {eff=219, desl = {x=1,y=0}}, 
    {eff=221, desl = {x=0,y=1}},
    {eff=220, desl = {x=1,y=0}},
    {eff=222, desl = {x=0,y=1}}
}
...
function onCastSpell(creature, var)
  ...
  if doPlayerAddExaust(cid, "air", "whip", fireExausted.whip) == false then
    return false
  end
  if getPlayerHasStun(cid) then
    workAllCdAndAndPrevCd(cid, "air", "whip", nil, 1)
    return true
  end

  if getPlayerOverPower(cid, "air", true, true) then
    workAllCdAndAndPrevCd(cid, "air", "whip", nil, 1)
