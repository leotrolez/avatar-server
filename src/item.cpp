                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.abilities) {
			...
			if (it.abilities->stats[STAT_MAGICPOINTS]) {
				...
				s << "bend level " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
			}
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " bend level " << it.minReqMagicLevel << " or higher";
		}
	...
	if (item) {
		Item* citem = const_cast<Item*>(item);
		if (citem && citem->getElementalRuneType() && citem->getElementalRuneLevel()) {
			static std::string elementosList[] = {"Fire Rune", "Water Rune", "Air Rune", "Earth Rune"};
			static std::string levelList[] = {"I", "II", "III", "IV"};
			if (citem->getElementalRuneType() < 5 && citem->getElementalRuneLevel() < 5)
				s << std::endl << "[Elemental slot: " << elementosList[citem->getElementalRuneType()-1] << " " << levelList[citem->getElementalRuneLevel()-1] <<"].";
			else if(citem->getElementalRuneType() > 10 && citem->getElementalRuneType() < 14 && citem->getElementalRuneCharges() > 0) {
				static std::string elementosList[] = {"Energy Rune", "Ice Rune", "Metal Rune"};
				s << std::endl << "[Elemental slot: " << elementosList[citem->getElementalRuneType()-11] << " " << levelList[citem->getElementalRuneLevel()-1] <<", Charges: "<< citem->getElementalRuneCharges() <<"].";
			}
		}
	}
