...
bool Events::load()
{
	...
	for (auto eventNode : doc.child("events").children()) {
		...
		if (className == "Creature") {
			...
			} else if (methodName == "onCastSpell") {
				info.playerOnCastSpell = event;
			} else {
...
bool Events::eventPlayerOnMove(Creature* creature, Direction direction)
{
	// Creature:onMove(direction) or Creature.onMove(self, direction)
	if (info.creatureOnMove == -1) {
		return true;
	}
	...
	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.creatureOnMove, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.creatureOnMove);
...
bool Events::eventPlayerOnCastSpell(Player* player, const InstantSpell* spell)
{
	// Player:onCastSpell(spell) or Player.onCastSpell(self, spell)
	if (info.playerOnCastSpell == -1) {
		return true;
	}

	if (!scriptInterface.reserveScriptEnv()) {
		std::cout << "[Error - Events::eventPlayerOnCastSpell] Call stack overflow" << std::endl;
		return false;
	}

	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.playerOnCastSpell, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.playerOnCastSpell);

	LuaScriptInterface::pushUserdata<Player>(L, player);
	LuaScriptInterface::setMetatable(L, -1, "Player");

	LuaScriptInterface::pushInstantSpell(L, *spell);
