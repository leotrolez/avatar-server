...
bool MoveEvents::registerEvent(Event_ptr event, const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("itemid"))) {
		auto idList = explodeString(attr.as_string(), ";");

		for (const auto& idStr : idList) {
			auto intVector = vectorAtoi(explodeString(idStr, "-"));
			if (!intVector[0])
				continue;
			...
			if(intVector.size() > 1) {
				while(intVector[0] < intVector[1]) {
					addEvent(*moveEvent, ++intVector[0], itemIdMap);
				}
			}
		}
		...
		auto uidList = explodeString(attr.as_string(), ";");

		for (const auto& uidStr : uidList) {
			auto intVector = vectorAtoi(explodeString(uidStr, "-"));
			if (!intVector[0])
				continue;

			addEvent(*moveEvent, intVector[0], uniqueIdMap);
			if(intVector.size() > 1) {
				while(intVector[0] < intVector[1]) {
					addEvent(*moveEvent, ++intVector[0], uniqueIdMap);
				}
		...
		auto aidList = explodeString(attr.as_string(), ";");

		for (const auto& aidStr : aidList) {
			auto intVector = vectorAtoi(explodeString(aidStr, "-"));
			if (!intVector[0])
				continue;
		
			addEvent(*moveEvent, intVector[0], actionIdMap);
			if(intVector.size() > 1) {
				while(intVector[0] < intVector[1]) {
					addEvent(*moveEvent, ++intVector[0], actionIdMap);
				}
...
bool MoveEvents::hasEquipEvent(Item* item)
{
	...
	return (event = getEvent(item, MOVE_EVENT_EQUIP)) && !event->isScripted() && (event = getEvent(item, MOVE_EVENT_DEEQUIP)) && !event->isScripted();
}
...
ReturnValue MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	int32_t runeType = item && item->getElementalRuneType() ? item->getElementalRuneType() : 0;
	int32_t runeLevel = item && item->getElementalRuneLevel() ? item->getElementalRuneLevel() : 0;
	int runeSpeeds [] = {7, 10, 14, 17};
	if (runeType == 3 && runeLevel > 0 && runeLevel < 5) {
		g_game.changeSpeed(player, runeSpeeds[runeLevel - 1]);
	}
	...
	if (runeType == 1 && runeLevel > 0 && runeLevel < 5) {
		static int runeBuffs[] = {2, 3, 4, 5};
		player->setVarStats(STAT_MAGICDAMAGEPERCENT, runeBuffs[runeLevel - 1]);
	}
...
ReturnValue MoveEvent::DeEquipItem(MoveEvent*, Player* player, Item* item, slots_t slot, bool)
{
	...
	int32_t runeType = item && item->getElementalRuneType() ? item->getElementalRuneType() : 0;
	int32_t runeLevel = item && item->getElementalRuneLevel() ? item->getElementalRuneLevel() : 0;
	static int runeSpeeds[] = {7, 10, 14, 17};
	if (runeType == 3 && runeLevel > 0 && runeLevel < 5) {
		g_game.changeSpeed(player, - runeSpeeds[runeLevel - 1]);
	}
