...
void LuaScriptInterface::registerFunctions()
{
	...
	lua_register(luaState, "doCreatureSetMoveImmuneTime", LuaScriptInterface::luaDoCreatureSetMoveImmuneTime);
	lua_register(luaState, "getCreatureMoveImmuneTime", LuaScriptInterface::luaGetCreatureMoveImmuneTime);

	//uniform_random()
	lua_register(luaState, "uniform_random", LuaScriptInterface::luaUniformRandom);
	...
	registerEnum(COMBAT_AIRDAMAGE)
	registerEnum(COMBAT_METALDAMAGE)
	...
	registerMethod("Item", "hasEquipEvent", LuaScriptInterface::luaItemHasEquipEvent);
	...
	registerMethod("Player", "addMagicLevel", LuaScriptInterface::luaPlayerAddMagicLevel);
...
int LuaScriptInterface::luaDoCreatureSetMoveImmuneTime(lua_State* L)
{
	...
	creature->setMoveImmuneTime(getNumber<uint32_t>(L, 2, 0));
	pushBoolean(L, true);
	return 1;
}

int LuaScriptInterface::luaGetCreatureMoveImmuneTime(lua_State* L)
{
	Creature* creature = getCreature(L, 1);
	if (!creature) {
		reportErrorFunc(L, getErrorDesc(LUA_ERROR_CREATURE_NOT_FOUND));
	...
	lua_pushnumber(L, creature->getNoMoveImmuneTime());
	return 1;
}
...
int LuaScriptInterface::luaUniformRandom(lua_State* L)
{
	//uniform_random()
	if (lua_gettop(L) == 1) {
		lua_pushnumber(L, uniform_random(1, getNumber<uint32_t>(L, 1, 0)));
		return 1;
	}
	lua_pushnumber(L, uniform_random(getNumber<uint32_t>(L, 1, 0), getNumber<uint32_t>(L, 2, 0)));
	return 1;
}
...
{
	// Game.sendAnimatedText(message, position, color, [player])
	int parameters = lua_gettop(L);
	if (parameters < 3) {
		pushBoolean(L, false);
		return 1;
	...
	if (!spectators.empty()) {
		Game::addAnimatedText(spectators, message, position, color);
	} else {
...
int LuaScriptInterface::luaItemHasEquipEvent(lua_State* L)
{
	// item:hasEquipEvent()
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		pushBoolean(L, g_moveEvents->hasEquipEvent(item));
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaCreatureSetMovementBlocked(lua_State* L)
{
	...
	Creature* creature = getUserdata<Creature>(L, 1);
	if (creature) {
		creature->setMovementBlocked(getBoolean(L, 2), getNumber<uint32_t>(L, 3, 0));
		creature->setMoveImmuneTime(getNumber<uint32_t>(L, 4, 0));
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaCreatureHasCondition(lua_State* L)
{
	...
	if (lua_gettop(L) == 2) {
		pushBoolean(L, creature->getCondition(conditionType) != nullptr);
		return 1;
	}
...
int LuaScriptInterface::luaPlayerAddMagicLevel(lua_State* L)
{
	// player:addMagicLevel(points)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		auto oldLevel = player->magLevel;
		player->magLevel += getNumber<int32_t>(L, 2, 0);
		player->sendSkills();
		player->sendStats();
		player->sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You advanced to bend level {:d}.", player->magLevel));
		g_creatureEvents->playerAdvance(player, SKILL_MAGLEVEL, oldLevel, player->magLevel);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerGetStorageValue(lua_State* L)
{
	...
	if (player->getStorageValue(key, value)) {
		...
		int32_t intValue = stringTo<int32_t>(value);
		if (intValue != -1) {
...
int LuaScriptInterface::luaPlayerSetStorageValue(lua_State* L)
{
	...
	int32_t keyInt = stringTo<int32_t>(key);
	if (IS_IN_KEYRANGE(keyInt, RESERVED_RANGE)) {
		reportErrorFunc(L, fmt::format("Accessing reserved range: {:d}", keyInt));
		pushBoolean(L, false);
		return 1;
	}
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				pushBoolean(L, true);
			} else if (immunity == "air") {
				monsterType->info.damageImmunities |= COMBAT_AIRDAMAGE;
				pushBoolean(L, true);
			} else if (immunity == "metal") {
				monsterType->info.damageImmunities |= COMBAT_METALDAMAGE;
				pushBoolean(L, true);
	...
	if (weapon) {
		if (!getNumber<CombatType_t>(L, 2)) {
			...
			if (tmpStrValue == "earth") {
				...
				weapon->params.combatType = COMBAT_AIRDAMAGE;
			} else if (tmpStrValue == "metal") {
				weapon->params.combatType = COMBAT_METALDAMAGE;
	...
	if (weapon) {
		...
		if (!getNumber<CombatType_t>(L, 3)) {
			...
			if (tmpStrValue == "earth") {
				...
				it.abilities.get()->elementType = COMBAT_AIRDAMAGE;
			} else if (tmpStrValue == "metal") {
				it.abilities.get()->elementType = COMBAT_METALDAMAGE;
			} else if (tmpStrValue == "holy") {
