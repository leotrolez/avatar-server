...
class Item : virtual public Thing
{
	public:
		...
		int32_t getElementalRuneType() {
			const ItemAttributes::CustomAttribute* attr = getCustomAttribute("elementalrunetype");
			if (attr) {
				if (attr->value.type() == typeid(int64_t)) {
					return static_cast<int32_t>(boost::get<int64_t>(attr->value));
				}
			}
			return 0;
		}

		int32_t getElementalRuneLevel() {
			const ItemAttributes::CustomAttribute* attr = getCustomAttribute("elementalrunelevel");
			if (attr) {
				if (attr->value.type() == typeid(int64_t)) {
					return static_cast<int32_t>(boost::get<int64_t>(attr->value));
				}
		...
		int32_t getElementalRuneCharges() {
			const ItemAttributes::CustomAttribute* attr = getCustomAttribute("elementalrunecharges");
			if (attr) {
				if (attr->value.type() == typeid(int64_t)) {
					return static_cast<int32_t>(boost::get<int64_t>(attr->value));
				}
		...
		void setElementalRuneCharges(int32_t newCharge)
		{
			static std::string key = "elementalrunecharges";
			setCustomAttribute<int64_t>(key, newCharge);
		}
