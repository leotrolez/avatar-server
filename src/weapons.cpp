...
void Weapon::internalRuneEffect(Player* player, Item* item, Creature* target, int32_t runeType, bool isRanged) const
{
	if (!target->isMovementBlocked()) {
		int32_t runeLevel = item->getElementalRuneLevel();
		int rollDice = uniform_random(1, 100);
		if (runeType == 12 && OTSYS_TIME() > static_cast<int64_t>(target->getIceImmuneTime())) {
			static int chancesByLevel [] = {3, 6};
			int theChance = chancesByLevel[runeLevel-1];
			if (!isRanged)
				theChance *= 2;
			if (rollDice <= theChance) {
				target->setMovementBlocked(false, 2000);
				target->setMoveImmuneTime(3000);
				target->setIceImmuneTime(5000);
				Position effectPos = target->getPosition();
				effectPos.x++;
				effectPos.y++;
				g_game.addMagicEffect(effectPos, 138);
				SpectatorVec spectators;
				spectators.emplace_back(player);
				g_game.addAnimatedText(spectators, "Frozzen!", target->getPosition(), TEXTCOLOR_MAYABLUE);
			}
		} else if(runeType == 13) {
			static int chancesByLevel[] = {10, 20};
			int theChance = chancesByLevel[runeLevel-1];
			if (!isRanged)
				theChance *= 2;
			if (rollDice <= theChance) {
				Condition* condition = Condition::createCondition(CONDITIONID_COMBAT, CONDITION_PARALYZE, 3000, (int32_t)-((double)target->getBaseSpeed()*0.4));
				target->addCondition(condition);
				SpectatorVec spectators;
				spectators.emplace_back(player);
				g_game.addAnimatedText(spectators, "Slowed!", target->getPosition(), TEXTCOLOR_MAYABLUE);
			}
		}
	}
}
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	if (scripted) {
		...
		if (item) {
			int32_t runeCharges = item->getElementalRuneCharges();
			int32_t runeType = item->getElementalRuneType();
			if (runeType > 10 && runeType < 14 && runeCharges > 0) {
				CombatParams runeParams;
				CombatDamage runeDamage;
				runeDamage.origin = damage.origin;
				static CombatType_t runeCombats[] = {COMBAT_ENERGYDAMAGE, COMBAT_ICEDAMAGE, COMBAT_METALDAMAGE};
				if (item->getID() == 12872) {
					static ShootType_t shootEffects[] = {CONST_ANI_ENERGY, CONST_ANI_SMALLICE, static_cast<ShootType_t>(50)};
					runeParams.distanceEffect = shootEffects[runeType - 11];
				}
				runeParams.combatType = runeCombats[runeType - 11];
				runeDamage.primary.type = runeParams.combatType;
				int32_t elementalDamage = damage.primary.value / 4;
				if (runeType == 11) {
					runeParams.impactEffect = 147;
					elementalDamage = (int32_t)std::ceil((double)(elementalDamage) * 3.);
				} else if (runeType == 13) {
					elementalDamage = (int32_t)std::ceil((double)(elementalDamage) * 2.);
				}
				if (item->getElementalRuneLevel() <= 1 && elementalDamage >= 2)
					elementalDamage = (int32_t)std::ceil((double)(elementalDamage) / 2.);
				runeParams.blockedByArmor = false;
				runeParams.blockedByShield = false;
				if (target->getMonster()) {
					if (runeType != 11)
						internalRuneEffect(player, item, target, runeType, item->getID() != 12872 ? false : true);
					Combat::doTargetCombat(player, target, runeDamage, runeParams);
				} else {
					int32_t runeLevel = item->getElementalRuneLevel();
					player->addStorageValue("36296", std::to_string(runeLevel));
					player->addStorageValue("36295", "1");
					runeDamage.primary.value = elementalDamage;
					Combat::doTargetCombat(player, target, runeDamage, runeParams);
					player->addStorageValue("36295", "0");
				}
				runeCharges--;
				item->setElementalRuneCharges(runeCharges);
			}
		}
	
		if (item && item->getWeaponType() == WEAPON_AMMO) {
			Item* bow = player->getWeapon(true);
			if (bow) {
				int32_t runeCharges = bow->getElementalRuneCharges();
				int32_t runeType = bow->getElementalRuneType();
				if (runeType > 10 && runeType < 14 && runeCharges > 0) {
					CombatParams runeParams;
					CombatDamage runeDamage;
					runeDamage.origin = damage.origin;
					...
					if (runeType == 11) {
						runeParams.impactEffect = 147;
						elementalDamage = std::ceil<int32_t>((double)(elementalDamage) * 3.);
					} else if(runeType == 13) {
						elementalDamage = std::ceil<int32_t>((double)(elementalDamage) * 2.);
					}
					if (bow->getElementalRuneLevel() <= 1 && elementalDamage >= 2)
						elementalDamage = std::ceil<int32_t>((double)(elementalDamage) / 2.);
					runeParams.blockedByArmor = false;
					runeParams.blockedByShield = false;
					if (target->getMonster()) {
						if (runeType != 11)
							internalRuneEffect(player, bow, target, runeType, true);
						Combat::doTargetCombat(player, target, runeDamage, runeParams);
					} else {
						int32_t runeLevel = bow->getElementalRuneLevel() ? bow->getElementalRuneLevel() : 1;
						player->addStorageValue("36296", std::to_string(runeLevel));
						player->addStorageValue("36295", "2");
						Combat::doTargetCombat(player, target, runeDamage, runeParams);
						player->addStorageValue("36295", "0");
					}
					runeCharges--;
					bow->setElementalRuneCharges(runeCharges);
				}
			}
		}
	}
...
bool WeaponWand::configureEvent(const pugi::xml_node& node)
{
	...
	} else if (tmpStrValue == "air") {
		params.combatType = COMBAT_AIRDAMAGE;
	} else if (tmpStrValue == "metal") {
		params.combatType = COMBAT_METALDAMAGE;
	} else {
