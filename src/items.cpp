...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"increasemagic", ITEM_PARSE_INCREASEMAGIC},
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_INCREASEMAGIC: {
					abilities.stats[STAT_MAGICDAMAGE] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
			
				case ITEM_PARSE_INCREASEMAGICPERCENT: {
					abilities.stats[STAT_MAGICDAMAGEPERCENT] = pugi::cast<int32_t>(valueAttribute.value()) - 100;
					break;
				}
				...
				case ITEM_PARSE_ABSORBPERCENTELEMENTS: {
					...
					abilities.absorbPercent[combatTypeToIndex(COMBAT_AIRDAMAGE)] += value;
					abilities.absorbPercent[combatTypeToIndex(COMBAT_METALDAMAGE)] += value;
					break;
				}
