	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		if (creature->getCombatValues(min, max)) {
			...
			if (params.valueCallback) {
				...
				damage.primary.value += player->getVarStats(STAT_MAGICDAMAGE);
				damage.primary.value += damage.primary.value * (static_cast<double>(player->getVarStats(STAT_MAGICDAMAGEPERCENT)) / 100.f);
...
CombatType_t Combat::ConditionToDamageType(ConditionType_t type)
{
	switch (type) {
		...
		//case CONDITION_POISON: // TODO: earth = poison ?
...
ConditionType_t Combat::DamageToConditionType(CombatType_t type)
{
	switch (type) {
		...
		//case COMBAT_EARTHDAMAGE: // TODO: earth = poison ?
