...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			combat->setParam(COMBAT_PARAM_TYPE, COMBAT_AIRDAMAGE);
		} else if (tmpName == "metal") {
			combat->setParam(COMBAT_PARAM_TYPE, COMBAT_METALDAMAGE);
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					mType->info.damageImmunities |= COMBAT_AIRDAMAGE;
				} else if (tmpStrValue == "metal") {
					mType->info.damageImmunities |= COMBAT_METALDAMAGE;
			...
			} else if ((attr = immunityNode.attribute("air"))) {
				if (attr.as_bool()) {
					mType->info.damageImmunities |= COMBAT_AIRDAMAGE;
				}
			} else if ((attr = immunityNode.attribute("metal"))) {
				if (attr.as_bool()) {
					mType->info.damageImmunities |= COMBAT_METALDAMAGE;
				}
	...
	if ((node = monsterNode.child("elements"))) {
		for (auto elementNode : node.children()) {
			...
			} else if ((attr = elementNode.attribute("airPercent"))) {
				mType->info.elementMap[COMBAT_AIRDAMAGE] = pugi::cast<int32_t>(attr.value());
				if (mType->info.damageImmunities & COMBAT_AIRDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"air\" on immunity and element tags. " << file << std::endl;
				}
			} else if ((attr = elementNode.attribute("metalPercent"))) {
				mType->info.elementMap[COMBAT_METALDAMAGE] = pugi::cast<int32_t>(attr.value());
				if (mType->info.damageImmunities & COMBAT_METALDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"air\" on immunity and element tags. " << file << std::endl;
				}
