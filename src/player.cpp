...
void Player::addStorageValue(const std::string& key, int32_t value, const bool isLogin/* = false*/)
{
	std::string strValue;
	if (value != -1) {
		strValue = std::to_string(value);
	}
	addStorageValue(key, strValue, isLogin);
}

void Player::addStorageValue(const std::string& key, std::string strValue, const bool isLogin/* = false*/)
{
	int32_t value = stringTo<int32_t>(strValue);
	int32_t keyInt = stringTo<int32_t>(key);

	if (IS_IN_KEYRANGE(keyInt, RESERVED_RANGE)) {
		if (IS_IN_KEYRANGE(keyInt, OUTFITS_RANGE)) {
	...
	if (!strValue.empty()) {
		...
		int32_t oldValue = stringTo<int32_t>(oldStrValue);

		storageMap[key] = std::move(strValue);
...
bool Player::getStorageValue(const std::string& key, int32_t& value) const
{
	auto it = storageMap.find(key);
	if (it == storageMap.end()) {
		value = -1;
	...
	value = stringTo<int32_t>(it->second);
                             ...
                             bool checkDefense /* = false*/, bool checkArmor /* = false*/, bool field /* = false*/, bool ignoreResistances /* = false*/)
{
	...
	if (!ignoreResistances) {
		for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_AMMO; ++slot) {
			...
			int32_t runeType = item->getElementalRuneType() ? item->getElementalRuneType() : 0;
			int32_t runeLevel = item->getElementalRuneLevel() ? item->getElementalRuneLevel() : 0;
			if (runeType == 4 && runeLevel > 0 && runeLevel < 5) {
				static int runeBuffs[] = {10, 15, 20, 25};
				damage -= std::round(damage * (runeBuffs[runeLevel - 1] / 100.));
			}
...
void Player::genReservedStorageRange()
{
	...
	for (const OutfitEntry& entry : outfits) {
		storageMap[std::to_string(++base_key)] = std::to_string((entry.lookType << 16) | entry.addons);
	}
}
...
uint16_t Player::getVocationEmblem() const
{
	return vocation->getFromVocation(); // getEmblem
}
