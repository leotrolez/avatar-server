...
class Creature : virtual public Thing
{
	...
	public:
		...
		void setMovementBlocked(bool state, uint32_t time = 0) {
			if (time > 0) {
				const auto toTime = OTSYS_TIME() + time;
				if (toTime > movementBlockedEndTime) {
					movementBlockedEndTime = toTime;
				}
				if (state) {
		...
		bool isMovementBlocked() const {
			if (movementBlockedEndTime > OTSYS_TIME()) {
				return true;
			}
			return movementBlocked;
		}
		void setIceImmuneTime(uint32_t tempo)
		{
			if (tempo > 0 && OTSYS_TIME() + tempo > iceImmuneEndTime)
				iceImmuneEndTime = OTSYS_TIME() + tempo;
		}
		uint64_t getIceImmuneTime() const
		{
			return iceImmuneEndTime;
		}
		...
		void setMoveImmuneTime(uint32_t time) {
			if (time > 0 && OTSYS_TIME() + time > immuneEndTime) {
				immuneEndTime = OTSYS_TIME() + time;
			}
		}
		uint64_t getNoMoveImmuneTime() const {
			return immuneEndTime;
		}
	...
	protected:
		...
		int64_t immuneEndTime = 0;
		int64_t iceImmuneEndTime = 0;
		int64_t movementBlockedEndTime = 0;
