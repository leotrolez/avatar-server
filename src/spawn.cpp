...
void Spawn::startSpawnMonster(uint32_t spawnId, spawnBlock_t& sb, uint8_t count)
{
	if (count == 0) {
	...
	g_scheduler.addEvent(createSchedulerTask(1000, std::bind(&Spawn::startSpawnMonster, this, spawnId, sb, --count)));
}
...
void Spawn::checkSpawn()
{
	...
	for (auto& it : spawnMap) {
		...
		if (time >= sb.lastSpawn + sb.interval) {
			sb.lastSpawn = time;
			startSpawnMonster(spawnId, sb, findPlayer(sb.pos) ? 6 : 3);
