...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT `key`, `value` FROM `player_storage` WHERE `player_id` = {:d}", player->getGUID())))) {
		do {
			player->addStorageValue(result->getString("key"), result->getString("value"), true);
		} while (result->next());
	}
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	for (const auto& it : player->storageMap) {
		if (!storageQuery.addRow(fmt::format("{:d}, {:s}, {:s}", player->getGUID(), db.escapeString(it.first), db.escapeString(it.second)))) {
			return false;
		}
	}
