...
class Mission
{
	public:
		Mission(std::string name, int32_t storageID, int32_t startValue, int32_t endValue, bool ignoreEndValue) :
			name(std::move(name)), storageID(std::to_string(storageID)), startValue(startValue), endValue(endValue), ignoreEndValue(ignoreEndValue) {}
...
class Quest
{
	public:
		Quest(std::string name, uint16_t id, int32_t startStorageID, int32_t startStorageValue) :
			name(std::move(name)), startStorageID(std::to_string(startStorageID)), startStorageValue(startStorageValue), id(id) {}
		...
		const std::string& getStartStorageId() const {
...
class Quests
{
	public:
		...
		bool isQuestStorage(const std::string& key, const int32_t value, const int32_t oldValue) const;
